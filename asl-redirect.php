...
function asl_redirect_settings()
{
    ...
    $asl_redirect_fields = array(
        ...
        'vals' => array(
            ...
            'none' => __('Без редиректа', 'asl-redirect'),
        )
    );
...
$address = (is_int(strpos($address, 'https://')) || is_int(strpos($address, 'http://'))) ? $address : 'https://' . $address;
if ($address[-1] !== '/') {
    $address .= '/';
}
/**
...
switch ($radio_options) {
    case('all'):
        add_action('template_redirect', function () use ($post, $address) {
            if (!is_front_page()) {
                wp_redirect(extracted($address, $post), 301);
            }
